        ...
        try:
            ...
            def _save():
                with open (_SAVE_PATH, "w") as f:
                    ...
                    # THE 4 functions this has to be updated later (*notes this is js for the sys)
...
def advance_day()
    ...
    for i, entry in enumrate(_state["plots"]);
        if entry is None:
            continue
        if weather == "storm":
            risk = SHOP[entry["crop"]]["risk"]
            if random.random() < risk:
                events.append(f"store destroyed the {entry['crop']} in plot {i + 1}.")
                _state["plots"][i] = None
                continue
            ...
            def get_state():
                ...
                return {
                    ...
                    "shop": {k: dict(v) for k, v in SHOP.itmes()},
                }
